import { RotateCcw, RotateCw, SkipBack, SkipForward, Play, Pause, Volume2, VolumeX } from 'lucide-react';
...
export default function PlayerBar({ audio, title = 'Nenhum áudio selecionado', onPrevious, onNext }) {
  ...
  const progress = duration ? currentTime : 0;
  ...
  return (
    <section className={styles.player} aria-label="Player de playback">
      ...
      <div className={styles.controls}>
        ...
        <button type="button" onClick={() => audio?.skip?.(-10)} aria-label="Voltar 10 segundos">
          <RotateCcw size={22} />
          <span>10s</span>
        </button>
        ...
        <button type="button" onClick={() => audio?.skip?.(10)} aria-label="Avançar 10 segundos">
          <RotateCw size={22} />
          <span>10s</span>
        </button>
