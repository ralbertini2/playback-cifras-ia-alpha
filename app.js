  ...
  try{
    ...
    const styleResults = await Promise.all(styles.map(async style=>{
      const files = await listAll({
        q: `'${style.id}' in parents and trashed=false and mimeType!='application/vnd.google-apps.folder'`,
        fields:'nextPageToken, files(id,name,mimeType,webViewLink,webContentLink,createdTime,modifiedTime)',
        orderBy:'name',
        pageSize:'1000'
      });
...
function pairFiles(style, files){
  ...
  for(const file of files){
    ...
    if(ext !== 'pdf' && !audioExts.has(ext)) continue;
    const base = normalizeBase(file.name);
    const key = normalizeMatchKey(base);
    if(!map.has(key)) map.set(key,{title:base,style:style.name,styleId:style.id, createdTime:file.createdTime || '', modifiedTime:file.modifiedTime || ''});
    const item = map.get(key);
    if(ext==='pdf') item.pdf = file;
    if(audioExts.has(ext)) item.audio = file;
  }
  return [...map.values()].filter(x=>x.pdf && x.audio).map(x=>({
    id:`${x.styleId || x.style}|${x.title}`,
    title:x.title,
    artist:'',
    album:'',
    key:'',
    ...
    tags:[],
    favorite:false,
    addedAt:x.createdTime || new Date().toISOString(),
    cover:null,
    style:x.style,
    styleId:x.styleId,
    pdfId:x.pdf.id,
    ...
    mp3Url:`https://www.googleapis.com/drive/v3/files/${x.audio.id}?alt=media`,
    files:{
      cifra:x.pdf.id,
      pdf:x.pdf.id,
      playback:x.audio.id,
